...
# Request broker IP
read -p "Hostname Server: " BROKER_IP
...
openssl req -new -x509 -days "$DAYS" -extensions v3_ca -key conf/certs/ca-"$NAME_SERVER".key -out conf/certs/ca-"$NAME_SERVER".crt -subj "/C=PE/ST=Arequipa/L=Arequipa/O=Sacha/OU=CA/CN=${BROKER_IP}" #-addext "subjectAltName=IP:${BROKER_IP}"
...
openssl req -new -out conf/certs/server-"$NAME_SERVER".csr -key conf/certs/server-"$NAME_SERVER".key -subj "/C=PE/ST=Arequipa/L=Hunter/O=Sacha/OU=Server/CN=${BROKER_IP}" -addext "subjectAltName=IP:${BROKER_IP}"
