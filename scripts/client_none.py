...
async def mqtt_publish(key, node, val, mqtt_client, reconnector):
    ...
    try:
        ...
        await mqtt_publish(key, node, val, mqtt_client, reconnector)

# Clase manejadora de suscripción OPC UA
class SubHandler:
    def __init__(self, write_api, bucket, org, mqtt_client, reconnector, timeout=60):
        self.last_val = {}
        self.timer_task = {}
    ...
    async def datachange_notification(self, node, val, data):
        ...
        if key not in self.last_val or val != self.last_val[key]:
            ...
            await self.write_api.write(bucket=self.bucket, org=self.org, record=point)
            await mqtt_publish(key, node, val, self.mqtt_client, self.reconnector)
...
async def main():
    ...
    async with InfluxDBClientAsync(url=INFLUXDB_CONFIG["url"], token=INFLUXDB_CONFIG["token"], org=INFLUXDB_CONFIG["org"]) as influx_client:
        ...
        try: 
            ...
            handler = SubHandler(write_api, INFLUXDB_CONFIG["bucket"], INFLUXDB_CONFIG["org"], mqtt_client, reconnector)
