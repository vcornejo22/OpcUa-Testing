...
class OPCUAClient:
    def __init__(self, server_address, user, password):
        ...
        self.client = Client(url=server_address)
    ...
    async def connect(self, cert, key, uri, number_nodes, period):
        try:
            ...
            var_list = [self.client.get_node(f"ns=3;i={i}") for i in range(1001, number_nodes)]
            handler = SubHandler()
...
class ProcessMonitor:
    ...
    async def read_process(self, csv_filename):
        flag = True
        count = 0
        memory_init = 0
        try:
            while count < self.timeout + 20:
                ...
                aux = pd.DataFrame.from_dict([{'timestamp': current_time, 'cpu_usage': cpu_usage, 'memory_usage': memory_end}])
                if flag:
                    self.df = aux
            ...
            self.df.to_csv(f"data/{csv_filename}.csv", index=False)
        return datetime.now()
...
async def main():
    ...
    for generation in range(int(GENERATIONS)):
        ...
        for individual in population:
            ...
            pid = int(PID)
            result = await asyncio.gather(opc_client.connect(UA_CERT, UA_KEY, UA_URI, number_nodes, period), process_monitor.read_process(label))
